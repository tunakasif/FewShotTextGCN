...
def text_to_utf8(in_path, out_path):
    ...
    for line in in_lines:  # sys.stdin:
        #    if i%2==0:
        ...
        for token in line:
            ...
            for i in range(int(num)):
                if lasttoken == " " and i == 0:
                    ...
                    ch = str(
                        byteVocab[str(256 + (base16decode(tk[2 * i : 2 * i + 2])))]
                    )
...
if __name__ == "__main__":
    ...
    # In vars
    ##################################################
    in_path = "data/mldoc_german_train.tsv"
    out_bytes_path = "out_bytes.txt"

    # SPM
    vocab_size = 1000
    model_prefix = "m"
    vocab_bytes_name = "vocab_bytes.txt"

    # Note to self: text must be tokenized and present in one file (train + test)

    ##################################################
    # Let's execute
    ##################################################

    print("converting text tu bytes")
    text_to_utf8(in_path, out_bytes_path)

    print("Starting SPM trainer")
    spm.SentencePieceTrainer.train(
        input=out_bytes_path,
        model_prefix=model_prefix,
        vocab_size=vocab_size,
        model_type="bpe",
    )
    print("Finished SPM trainer")

    os.rename(f"{model_prefix}.vocab", vocab_bytes_name)
