...
r8 | r52 | mr | ohsumed | 20ng | cola | sst2)
	...
	for item in "${dev_splits[@]}"; do
		python main.py -c experiment_params/self_training.ini --experiment_name "$dataset" --path_to_train_set data/"$dataset"-train.tsv --path_to_test_set data/"$dataset"-test.tsv --tokenizer_type whitespace --language english --percentage_dev "$item" --num_seeds 1 --max_epochs 1000 "$is_mcc"
	done
	;;
*)
